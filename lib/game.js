...
function gameLoad(){
  ...
  // const play = canvas.getElementById("play")
...
function checkRockCollision(skier, rocks){
  ...
  for (var i = 0; i < numRocks; i++) {
    ...
    if ((skier.x < rock.x + 40 && skier.x + 50 > rock.x) &&
        (skier.y < rock.y + 30 && skier.y + 50 > rock.y)){
          skierHitObject();
      }
    }
  }
...
function checkBearCollision(skier, bears){
  const numBears = bears.length;
    for (var i = 0; i < numBears; i++) {
      ...
      if ((skier.x < bear.x + 40 && skier.x + 50 > bear.x) &&
          (skier.y < bear.y + 40 && skier.y + 50 > bear.y)){
            skierHitObject();
          }
      }
}
